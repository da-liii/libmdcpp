...
bool isCodeFenceBeginLine(const string& line, int& indent, int& length, char& fence, string& info) {
    ...
    if (fence!='`' && fence!='~')
        return false;
    while (si!=sie && *si==fence) { si++; length++; }
    if (length < 3)
        return false;
    if (si==sie) 
        return true;
...
bool isCodeFenceEndLine(const string& line, int indent, int openLen, char fence, std::ostringstream& out) {
    int maxIndent = 0;
    auto si = line.begin(), sie = line.end();
    while (si!=sie && *si==' ' && indent) { si++; indent--; maxIndent++; }
    auto sii = si;
    while (si!=sie && *si==' ' && maxIndent < 4) { si++; maxIndent++; }
    if (maxIndent > 3) {
        out << string(sii, sie) << "\n";
        return false;
    }
    ...
    int closeLen = 0;
    while (si!=sie && *si==fence) { si++; closeLen++; }
    if (closeLen < openLen) {
        out << string(sii, sie) << "\n";
        return false;
    }
